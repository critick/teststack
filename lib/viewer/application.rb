...
require "viewer/feature"
require "viewer/project"
require "viewer/search_features"
require "viewer/counts_tags"
...
def current_project
  @current_project ||= Viewer::Project.first(:name => params[:project])
end

def tag_count
  return {} if current_project.nil?
  @tag_count ||= Viewer::CountsTags.new(current_project).count_tags
end
...
def authenticated?
  File.exist?(".viewer") && params[:authentication_code] == File.read(".viewer").strip
end
...
put '/projects/:project/features/?' do
  ...
  project = Viewer::Project.create(:name => params[:project])

  JSON.parse(request.body.read).each do |json|
    project.features << Viewer::Feature.new(:path => json["path"], :gherkin => json["gherkin"])
  end
  project.save
  halt 201
end
...
delete '/projects/:project' do
  error 403 unless authenticated?
  project = Viewer::Project.first(:name => params[:project])
  project.destroy
  halt 201
end
...
get '/projects/:project/search/?' do
  if params[:q]
    @search_results = Viewer::SearchFeatures.new(current_project).find(:query => params[:q])
  end
  haml :search
end
